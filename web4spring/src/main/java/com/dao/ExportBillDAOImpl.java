...
@Repository
public class ExportBillDAOImpl implements ExportBillDAO{
	@Autowired
	...
	public void createPDF(int invno, String invdate) {
		try {
			...
			PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("C:\\Users\\Prethikaa Madhan\\eclipse-workspace\\web4spring\\"+invno+".pdf"));
		    document.open();
			...
			Document doc=db.parse("C:\\Users\\Prethikaa Madhan\\eclipse-workspace\\web4spring\\"+invno+".xml");
	...
	public void createExcel(int invno, String invdate) {
		...
		OutputStream fileOut = new FileOutputStream("C:\\Users\\Prethikaa Madhan\\eclipse-workspace\\web4spring\\"+invno+".xls");   
	...
	public void sendMail(String name,String email,int invno) {
		...
		message.setFrom(new InternetAddress("prethikaam@gmail.com")); 
		...
		attachmentPart.attachFile(new File("C:\\Users\\Prethikaa Madhan\\eclipse-workspace\\web4spring\\"+invno+".pdf"));
	...
	public void createXML(int invno, AllDetails alldetails) {
		try {
			...
			sb.append("<customerno>"+alldetails.getCustomer().getCustomerId()+"</customerno><customername>"+alldetails.getCustomer().getCustomerName()+"</customername><customeraddress>"+
			alldetails.getCustomer().getCustomerAddress()+"</customeraddress><customeremail>"+alldetails.getCustomer().getCustomerEmail()+"</customeremail><customerphone>"+
			alldetails.getCustomer().getCustomerPhone()+"</customerphone><invoiceno>"+invno+"</invoiceno></customer>");
			sb.append("<items>");
			//for (Item id : alldetails.getHm()) {
			HashMap<Item,Integer> hm = alldetails.getHm();
			 for (Map.Entry<Item,Integer> entry : hm.entrySet()) {
				sb.append("<item>"); 
		       sb.append("<itemno>"+entry.getKey().getItemno()+"</itemno>");       
		       sb.append("<itemdescription>"+entry.getKey().getItemdescription()+"</itemdescription>"
		       		+ "<itemprice>"+entry.getKey().getItemprice()+"</itemprice><itemqty>"+entry.getValue()+"</itemqty><itemunit>"+
		       		entry.getKey().getItemunit()+"</itemunit>"+"<itemtotal>"+entry.getKey().getItemprice() * entry.getValue()+"</itemtotal>");
		       sb.append("</item>");
			}
			sb.append("</items></invoice>");

					File output = new File("C:\\Users\\Prethikaa Madhan\\eclipse-workspace\\web4spring\\"+invno+".xml");
	...
	public void sendSMS(String phone, AllDetails alldetails) {
		final String ACCOUNT_SID = "AC8145a883bd274e99356d8f6215a966fa";
		final String AUTH_TOKEN = "b408ec0bdec680a68cbacd020738ad25";
		Twilio.init(ACCOUNT_SID, AUTH_TOKEN);
		String to="";
		String message="";
		to="+919840780854";
		message="Hello "+alldetails.getCustomer().getCustomerName()+"\n";
		StringBuilder sb=new StringBuilder();
		sb.append("Items:"+"\n");
		int i=1;
		//for (Item id : alldetails.getHm()) {
		HashMap<Item,Integer> hm = alldetails.getHm();
		 for (Map.Entry<Item,Integer> entry : hm.entrySet()) {
		   sb.append("Item "+i+" :"+"\n"); 
	       sb.append("Item No:"+entry.getKey().getItemno()+"\n");       
	       sb.append("Item Description: "+entry.getKey().getItemdescription()+"\n"
	       		+ "Item Price: "+entry.getKey().getItemprice()+"\n"+"Item Quantity: "+entry.getValue()+"\n"+"Item Unit: "+
	       		entry.getKey().getItemunit()+"\n"+"Iitemtotal: "+entry.getKey().getItemprice()*entry.getValue()	    		   );
	       sb.append("\n");
	       i++;
		}
